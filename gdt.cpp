...
GlobalDescriptorTable::GlobalDescriptorTable()
  ...
  uint32_t i[2]; // This stores the size and the base of the GDT, required by
                 // lgdt instruction

  i[1] = (uint32_t)this; // The last two bytes store the base address of the GDT
  i[0] = sizeof(GlobalDescriptorTable)
         << 16; // The first two store the size of the GDT.
...
GlobalDescriptorTable::SegmentDescriptor::SegmentDescriptor(uint32_t base,
  ...
  if (limit <= 65536) {
    ...
    target[6] =
        0xC0; // granularity bit is set, each unit is now interpreted as 4KB
  }
